    ...
    static void SDLCALL FeedTheAudioStreamMore(void *userdata, SDL_AudioStream *astream, int additional_amount,
        ...
        while (additional_amount > 0) {
            ...
            for (int i = 0; i < total; i++) {
                ...
                const float phase = static_cast<float>(current_sine_sample) * freq / 8000.0f;
            ...
            SDL_PutAudioStreamData(astream, samples, static_cast<int>(total * sizeof(float)));
...
int main(const int argc, char* argv[]) {
    ...
    if (argc > 3) {
        std::cerr << "Error: Invalid amount of arguments passed" << std::endl;
        std::exit(EXIT_FAILURE);
    }

    for (int i = 0; i < argc; i++) {
        if (std::string(argv[i]) == "./chip-8") continue;
        ...
        GAME_FILENAME = std::string(argv[i]);
    }
